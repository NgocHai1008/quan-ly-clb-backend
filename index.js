...
// --- API PROFILE (ĐÃ FIX AVATAR VÀ TÊN) ---
app.put('/api/profile', async (req, res) => {
    try {
        const { username, avatar, name: newName } = req.body; // Lấy avatar mới và tên mới từ body
        const oldUser = await User.findOne({ username });
        if (!oldUser) return res.status(404).json({ success: false });

        const oldName = oldUser.name; // Tên cũ (đang dùng làm author trong Post)

        // 1. Cập nhật User
        const updatedUser = await User.findOneAndUpdate({ username }, { name: newName, phone: req.body.phone, avatar: avatar }, { new: true });

        // 2. Cập nhật Student/Contacts
        // Cần đảm bảo rằng Student/Contacts vẫn được cập nhật từ tên cũ sang tên mới
        await Student.updateMany({ name: oldName }, { name: updatedUser.name });

        // 3. 🔥 FIX CUỐI CÙNG: Cập nhật tất cả Posts (tìm bằng tên cũ, cập nhật bằng tên mới và avatar mới)
        if (avatar) {
            await Post.updateMany(
                { author: oldName }, // <-- DÙNG TÊN CŨ ĐỂ TÌM
                { $set: { authorAvatar: avatar, author: updatedUser.name } } // Cập nhật cả avatar và tên tác giả mới
            );
        }

        res.json({ success: true, user: { ...updatedUser._doc, password: '' } });
...
// API: Đánh dấu tất cả thông báo của user là "Đã đọc"
app.put('/api/notifications/read-all', async (req, res) => {
    const { username } = req.body;
    const u = await User.findOne({ username: username });

    if (u) {
        // Cập nhật tất cả thông báo của user đó thành isRead: true
        await Notification.updateMany(
            { $or: [{ targetUser: u.name }, { targetUser: 'ALL' }], isRead: false },
            { $set: { isRead: true } }
        );
    }
    res.json({ success: true });
