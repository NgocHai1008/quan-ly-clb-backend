...
const processMemberImport = async (membersToImport, res) => {
    ...
    for (const member of membersToImport) {
        ...
        try {
            ...
            if (userExists) { results.push({ name: studentName, status: 'error', message: 'Tên đăng nhập đã tồn tại.' }); index++; continue; }
...
app.post('/api/admin/import-excel', async (req, res) => {
    ...
    if (!base64Data || !filename) return res.status(400).json({ success: false, message: 'Thiếu dữ liệu' });
    try {
        ...
        if (membersToImport.length === 0) return res.json({ success: false, results: [], message: 'Không tìm thấy tên.' });
        return processMemberImport(membersToImport, res);
    } catch (error) { return res.status(500).json({ success: false, message: 'Lỗi đọc Excel.' }); }
});
app.post('/api/admin/import-members', async (req, res) => { return processMemberImport(req.body.members, res); });

// 🔥 API QUẢN LÝ THÀNH VIÊN (MỚI) ---
// 1. Lấy danh sách đầy đủ (Join Student + User)
app.get('/api/admin/members-full', async (req, res) => {
    const students = await Student.find();
    const users = await User.find(); // Lấy tất cả user để map phone/avatar
    
    const fullList = students.map(s => {
        const u = users.find(user => user.name === s.name);
        return {
            id: s.id,
            name: s.name,
            username: u?.username || 'Chưa có TK',
            phone: u?.phone || 'Chưa cập nhật',
            avatar: u?.avatar || 'https://i.pravatar.cc/100?img=1',
            role: u?.role || 'member'
        };
    });
    res.json(fullList);
...
app.delete('/api/admin/members/:id', async (req, res) => {
    try {
        ...
        if (student) {
            // Xóa User tương ứng
            await User.deleteOne({ name: student.name });
            // Xóa Student
            await Student.deleteOne({ id: studentId });
            res.json({ success: true, message: 'Đã xóa thành viên và tài khoản.' });
        } else {
...
// --- API HỌC PHÍ & THÔNG BÁO ---
app.post('/api/tuition', async (req, res) => { 
    const { id, month } = req.body; 
    if (!month) return res.status(400).json({ success: false, message: 'Thiếu tháng.' });
...
app.post('/api/tuition/remind', async (req, res) => { 
    ...
    let qrData = null;
    try { qrData = JSON.parse(qrUrl); } catch (e) { qrData = { error: "Invalid QR" }; }
